...
export class SigridApi {
  ...
  getOpenSourceHealthFindings() {
    const configuration = this.configuration();
    return this.http.get<OpenSourceHealthResponse>(joinUrl(this.sigridApiBaseUrl(), 'osh-findings', configuration.customer, configuration.system));
  }

  getSecurityFindings() {
    const configuration = this.configuration();
    return this.http.get<SecurityFindingResponse[]>(joinUrl(this.sigridApiBaseUrl(), 'security-findings', configuration.customer, configuration.system));
  }

  getRefactoringCandidates(category: RefactoringCategory) {
    const configuration = this.configuration();
    return this.http.get<RefactoringCandidatesResponse>(joinUrl(this.sigridApiBaseUrl(), 'refactoring-candidates', configuration.customer, configuration.system, category));
  }
